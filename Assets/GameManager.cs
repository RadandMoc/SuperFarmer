...
public class GameManager : MonoBehaviour
{
    ...
    public void ThrowDices()
    {
        ...
        if (firstResult == secondResult)
        {
            if (PlayerList[ActualPlayerId].NumOfAnimals.TryGetValue(firstResult, out numOfAnimals))
            {
                ...
                WhatChangedRollDices += $"Added {addidtionalAnimals} of {firstResult.GetName()}s";
            ...
            else
            {
                if (GameManager.Instance.playerList[0].NumOfAnimals[firstResult] > 0)
                {
                    ...
                    WhatChangedRollDices += $"Added 1 {firstResult.GetName()}";
                    GameManager.Instance.playerList[0].NumOfAnimals[firstResult] -= 1;
                }
                else
                    WhatChangedRollDices += $"Bank don't have this type of animal";
            }
        }
        else
        {
            if (firstResult is Wolf || secondResult is Fox)
            {
                ...
                else if (firstResult is Wolf)
                {
                    if (WolfEat())
                    {
                        ...
                        {
                            if (PlayerList[ActualPlayerId].NumOfAnimals.TryGetValue(secondResult, out numOfAnimals))
                            {
                                int addidtionalAnimals2 = Math.Min((int)((numOfAnimals + 1) / 2), GameManager.Instance.playerList[0].NumOfAnimals[secondResult]);
                                PlayerList[ActualPlayerId].NumOfAnimals[secondResult] = numOfAnimals + addidtionalAnimals2;
                                WhatChangedRollDices += $"\nAdded {addidtionalAnimals2} of {secondResult.GetName()}s";
                                GameManager.Instance.playerList[0].NumOfAnimals[secondResult] -= addidtionalAnimals2;
                            }
                        }
                    }
                    else
                ...
                else //if (secondResult is Fox)
                {
                    if (FoxEat())
                    {
                        if (firstResult is not Rabbit)
                        {
                            if (PlayerList[ActualPlayerId].NumOfAnimals.TryGetValue(firstResult, out numOfAnimals))
                            {
                                int addidtionalAnimals = Math.Min((int)((numOfAnimals + 1) / 2), GameManager.Instance.playerList[0].NumOfAnimals[firstResult]);
                                PlayerList[ActualPlayerId].NumOfAnimals[firstResult] = numOfAnimals + addidtionalAnimals;
                                WhatChangedRollDices += $"\nAdded {addidtionalAnimals} of {firstResult.GetName()}s";
                                GameManager.Instance.playerList[0].NumOfAnimals[firstResult] -= addidtionalAnimals;
                            }
                        }
                    }
            ...
            else
            {
                ...
                if (PlayerList[ActualPlayerId].NumOfAnimals.TryGetValue(secondResult, out numOfAnimals))
                {
                    ...
                    int addidtionalAnimals2 = Math.Min((int)((numOfAnimals + 1) / 2), GameManager.Instance.playerList[0].NumOfAnimals[secondResult]);
                    PlayerList[ActualPlayerId].NumOfAnimals[secondResult] = numOfAnimals + addidtionalAnimals2;
                    WhatChangedRollDices += $"Added {addidtionalAnimals2} of {secondResult.GetName()}s";
                    GameManager.Instance.playerList[0].NumOfAnimals[secondResult] -= addidtionalAnimals2;
                }
                else if (!added)
                    WhatChangedRollDices += "Nothing has changed";
            }
        }
    }
    ...
    private bool FoxEat()
    {
        ...
        {
            int numOfAnimals = 0;
            if (PlayerList[actualPlayerId].NumOfAnimals.TryGetValue(new Rabbit(), out numOfAnimals))
            {
                PlayerList[actualPlayerId].NumOfAnimals[new Rabbit()] = 1;
                WhatChangedRollDices += $"You lost {numOfAnimals - 1} rabbits";
                PlayerList[0].NumOfAnimals[new Rabbit()] += numOfAnimals - 1;
            }
            else
                WhatChangedRollDices += $"The fox did nothing to the player's farm.";
            return true;
        }
        else // disapear small dog
        {
            ...
            WhatChangedRollDices += "You lost your small dog.";
            return false;
        }
    }
    ...
    private bool WolfEat()
    {
        ...
        if (!PlayerList[actualPlayerId].NumOfAnimals.TryGetValue(new BigDog(), out numOfDogs)) // player don't have any big dog
        {
            ...
            if (PlayerList[actualPlayerId].NumOfAnimals.TryGetValue(new Sheep(), out numOfAnimals))
            {
                ...
                WhatChangedRollDices += $"You lost {numOfAnimals} sheeps\n";
            ...
            if (PlayerList[actualPlayerId].NumOfAnimals.TryGetValue(new Cow(), out numOfAnimals))
            {
                ...
                WhatChangedRollDices += $"You lost {numOfAnimals} cows\n";
            ...
            if (PlayerList[actualPlayerId].NumOfAnimals.TryGetValue(new Pig(), out numOfAnimals))
            {
                ...
                WhatChangedRollDices += $"You lost {numOfAnimals} pigs\n";
                PlayerList[0].NumOfAnimals[new Pig()] += numOfAnimals;
                somethingChanged = true;
            }
            if(!somethingChanged)
                WhatChangedRollDices += $"The wolf did nothing to the player's farm.";
            return true;
        }
        else // disapear big dog
        {
            ...
            WhatChangedRollDices += "You lost your big dog.";
            return false;
        }
    }
